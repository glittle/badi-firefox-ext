...
if (_notificationsEnabled) {
    // in FF, need to try to use speechSynthesis before it will load voices
    // do this now, so that we can catch when they load
    ...
    Notification.requestPermission().then(function(result) {
        ...
        console.log('notification response catch', reason);
    });
    ...
    //   if (level !== 'granted') {
    //     _notificationsEnabled = false;
    //     console.log('notications denied');
    //   }else{
...
var BackgroundReminderEngine = function(_alarmHandler) {
    var _ports = [];
    var _reminderPrefix = 'alarm_';
    var _dayChangePrefix = 'reminder_';
    var _specialDays = {};
    ...
    function buildUpAlarmInfo(alarmInfo, testDayDi, holyDayInfo) {
        ...
        // console.log('build alarm 1', alarmInfo)
    ...
    var createAlarm = function(alarmInfo, isTest, prefix) {
        clearTimeout(alarmInfo.timer);

        var delay = alarmInfo.triggerTime - new Date().getTime();

        // console.log('setting timeout', alarmInfo);
        alarmInfo.timer = setTimeout(function() {
            console.log('timeout running', _alarmHandler, alarmInfo);
            _alarmHandler(alarmInfo.key);
        }, delay, alarmInfo.name);

        storeAlarmReminder(alarmInfo, isTest, prefix || _reminderPrefix);
    }
    ...
    var setDayChangeAlarm = function(reminder) {
        var oldAlarm = getStorage(_dayChangePrefix + reminder.name);
        if (oldAlarm) {
            alarm = oldAlarm;
            alarm.triggerTime = reminder.triggerTime;
        } else {
            alarm = reminder;
        }

        createAlarm(alarm, false, _dayChangePrefix);
    }
    ...
    function doAdditionalActions(alarmInfo) {
        ...
        switch (alarmInfo.action) {
            ...
            //   break;
            case 'speak':
                //using Web API (Firefox)
                var utterance = new SpeechSynthesisUtterance(voiceContent)
                var voices = window.speechSynthesis.getVoices();
                for (var i = 0; i < voices.length; i++) {
                    if (voices[i].name === alarmInfo.speakVoice) {
    ...
    function clearReminderAlarms(fnAfter) {
        for (let i = 0; i < localStorage.length; i++) {
            ...
            if (key.startsWith(_reminderPrefix)) {
                // console.log('clear alarm', key);
                var alarm = getStorage(key);
    ...
    function dumpAlarms() {
        browser.alarms.getAll(function(alarms) {
            for (var i = 0; i < alarms.length; i++) {
                ...
                console.log(alarm.name, getStorage(alarm.name));
            }
        });
    }
        ...
        chrome.runtime.onConnect.addListener(function(port) {
            ...
            port.onMessage.addListener(function(msg) {
                ...
                switch (msg.code) {
                    ...
                    case 'setReminderAlarm':
                        setReminderAlarm(msg.reminder);
                        break;
