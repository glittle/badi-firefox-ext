    ...
    function buildMonth(gYear, gMonth) {
        ...
        while (!isNaN(gDate)) {
            ...
            if (!outside) {
                ...
                week.push(['<div class="outside" id=o{cellId}><div style="height:{outsideHeight}px">', '<div class=morn>' +
                    '<span class=gDay>{frag2Day}</span></div>', '<div class=aft></div>', '<div class=eve></div>', '</div></div>'
                ].join('').filledWith(thisDayInfo))
            }
