...
var _remindersEnabled = true; // browserHostType === browserType.Chrome;
...
function fillSetup() {
    ...
    if (!pct || lang === 'en') {
        ...
        var msg = _rawMessageTranslationPct === 100 ?
            ...
            getMessage('setupLangPct').filledWith(pct);
...
function startFillingLanguageInput(select) {
    ...
    for (i = 0; i < langs.length; i++) {
        var info = langs[i];
        options.push('<option value={0}>{1} ... {0}</option>'.filledWith(info.code, info.name))
    }
